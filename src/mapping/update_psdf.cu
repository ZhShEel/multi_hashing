...
void PredictOutlierRatioKernel(
  ...
  for (int i = 0; i < N_VERTEX; ++i) {
    if (this_mesh_unit.vertex_ptrs[i] > 0) {
      Vertex& vtx = mesh_m.vertex(this_mesh_unit.vertex_ptrs[i]);
...
void UpdateBlocksBayesianKernel(
  ...
  float tau = squaref(depth - 0.4f) * 0.0012f + 0.0019f;
  // for ICL:
  //float tau = 0.004f * 35130.0f / (35130.0f / fmaxf(depth, 0.4f) + 0.66f);
  float truncation = (geometry_helper.voxel_size + tau) * 3;
  if (x <= -truncation)
    return;
  if (x >= 0.0f) {
  ...
  if (this_voxel.inv_sigma2 == 0) {
    ...
    this_voxel.color.x = (unsigned char)(int)(255*color.x);
    this_voxel.color.y = (unsigned char)(int)(255*color.y);
    this_voxel.color.z = (unsigned char)(int)(255*color.z);
    ...
    this_voxel.color.y = (unsigned char)(int)(255*color.y);
    this_voxel.color.z = (unsigned char)(int)(255*color.z);
      //printf("%f %f %f %c %c %c\n",color.x,color.y,color.z,this_voxel.color.x,this_voxel.color.y,this_voxel.color.z);
  }
}
...
void BuildSensorDataEquationKernel(
  ...
  for (int i = 0; i < N_VERTEX; ++i) {
    if (mesh_unit.vertex_ptrs[i] > 0) {
      Vertex vtx = mesh_m.vertex(mesh_unit.vertex_ptrs[i]);
      float3 v = vtx.pos;
      float3 n = vtx.normal;
