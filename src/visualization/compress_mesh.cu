...
void CollectVerticesAndTrianglesKernel(
  ...
  for (int i = 0; i < N_TRIANGLE; ++i) {
    int triangle_ptrs = mesh_unit.triangle_ptrs[i];
      if(triangle_ptrs>=6000000){
          return;
        printf("out:%d\n",triangle_ptrs);
      }
    if (triangle_ptrs != FREE_PTR) {
      int3& triangle = mesh_m.triangle(triangle_ptrs).vertex_ptrs;
...
__global__
void CompressVerticesKernel(Mesh        mesh_m,
                            CompactMesh compact_mesh,
  ...
  if (addr_local != -1) {
    ...
    compact_mesh.vertices()[addr] = mesh_m.vertex(idx).pos;
    compact_mesh.normals()[addr]  = mesh_m.vertex(idx).normal;
    compact_mesh.colors()[addr]   = mesh_m.vertex(idx).color;

    atomicAdd(vertex_ref_count, mesh_m.vertex(idx).ref_count);
  }
}

__global__
void CompressTrianglesKernel(Mesh        mesh_m,
                             CompactMesh compact_mesh,
  ...
  if (addr_local != -1) {
    const uint addr = addr_global + addr_local;
    int3 vertex_ptrs = mesh_m.triangle(idx).vertex_ptrs;
    ...
    compact_mesh.scales()[addr] = mesh_m.triangle(idx).scale;
...
void CompressMesh(EntryArray& candidate_entries,
...

  {
    ...
    const dim3 grid_size((mesh_m.params().max_vertex_count
                          + threads_per_block - 1)
                         / threads_per_block, 1);
    const dim3 block_size(threads_per_block, 1);
    ...
    CompressVerticesKernel <<< grid_size, block_size >>> (
        mesh_m,
            compact_mesh,
            mesh_m.params().max_vertex_count,
            vertex_ref_count);
    checkCudaErrors(cudaDeviceSynchronize());
    checkCudaErrors(cudaGetLastError());
  ...
  {
    const uint threads_per_block = 256;
    const dim3 grid_size((mesh_m.params().max_triangle_count
                          + threads_per_block - 1)
                         / threads_per_block, 1);
    const dim3 block_size(threads_per_block, 1);
    ...
    CompressTrianglesKernel <<< grid_size, block_size >>> (
        mesh_m,
            compact_mesh,
            mesh_m.params().max_triangle_count);
    checkCudaErrors(cudaDeviceSynchronize());
    checkCudaErrors(cudaGetLastError());
  }

  LOG(INFO) << "Vertices: " << compact_mesh.vertex_count()
            << "/" << (mesh_m.params().max_vertex_count - mesh_m.vertex_heap_count());
  stats.x = compact_mesh.vertex_count();

  LOG(INFO) << "Triangles: " << compact_mesh.triangle_count()
            << "/" << (mesh_m.params().max_triangle_count - mesh_m.triangle_heap_count());
  stats.y = compact_mesh.triangle_count();
