...
inline bool GetSpatialValue(
  ...
  const float3 pos_corner = pos - 0.5f * offset; // sdf value is stored at the center of the voxel
...
#pragma unroll 1
  for (int i = 0; i < 8; ++i) {
    ...
    float3 r = (make_float3(1.0f) - mask) * (make_float3(1.0) - ratio)
               + (mask) * ratio;
    printf("pos_corner:%f %f %f mask:%f %f %f r:%f %f %f\n",pos_corner.x,pos_corner.y,pos_corner.z,mask.x,mask.y,mask.z,r.x,r.y,r.z);
