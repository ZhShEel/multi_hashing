...
void Mesh::Alloc(const MeshParams &mesh_params) {
  if (!is_allocated_on_gpu_) {
    ...
    //printf("max_vetex_count %d", sizeof(uint) * mesh_params.max_triangle_count);
    ...
    checkCudaErrors(cudaMemset(vertices,FREE_PTR,sizeof(Vertex)*mesh_params.max_vertex_count));
    checkCudaErrors(cudaMemset(triangles,FREE_PTR,sizeof(Triangle)*mesh_params.max_triangle_count));
    //printf("size of vertex: %zu, of traingle: %zu", sizeof(Vertex), sizeof(Triangle));
