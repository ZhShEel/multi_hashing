...
void LoadRuntimeParams(std::string path, RuntimeParams& params) {
  ...
  params.color_type = (int)fs["color_type"];
  ...
  params.enable_meshing = (int)fs["enable_meshing"];
  params.enable_ray_casting   = (int)fs["enable_ray_casting"];
  params.enable_join_process  = (int)fs["enable_join_process"];
  params.enable_view_rotation = (int)fs["enable_view_rotation"];

  params.enable_video_recording  = (int)fs["enable_video_recording"];
  params.enable_ply_saving     = (int)fs["enable_ply_saving"];
  params.save_result_all_the_time = (int)fs["save_result_all_the_time"];
  ...
  params.enable_gl_vis   = (int)fs["enable_gl_vis"];
  params.save_per_frame  = (int)fs["save_per_frame"];
...
void LoadScaleTableParams(std::string path, ScaleParams& params) {
  cv::FileStorage fs(path, cv::FileStorage::READ);
  params.bucket_count     = (int)fs["scale_bucket_count"];
  params.bucket_size      = (int)fs["scale_bucket_size"];
  params.entry_count      = (int)fs["scale_count"];
  params.linked_list_size = (int)fs["scale_linked_list_size"];
  params.value_capacity   = (int)fs["scale_value_capacity"];
  params.curvature_th     = (float)fs["curvature_th"];
  params.cd_th            = (float)fs["cd_th"];
}
...
void LoadVolumeParams(std::string path, VolumeParams& params) {
  ...
  params.approximate_size          = (float)fs["approximate_size"];
  ...
  params.regularization_rough_measure = (float)fs["regularization_rough_measure"];
  params.join_th                   = (float)fs["join_th"];
}
...
void LoadZHOU(std::string dataset_path,
  ...
  while (color_stream >> img_name) {
    color_img_list.push_back(dataset_path + "/" + img_name);
  }

  std::ifstream depth_stream(dataset_path + "depth.txt");
  while (depth_stream >> img_name) {
    depth_img_list.push_back(dataset_path + "/" + img_name);
  }
...
const std::string kConfigPaths[] = {
    ...
    "../config/ply3.yml",
    "../config/B_n_P.yml",
    "../config/Zhou_chair.yml",
    "../config/Zhou_misc.yml",
    "../config/Zhou_trash_container.yml",
    "../config/Test.yml",
    "../config/Dragon.yml",
    "../config/Element.yml",
    "../config/ShapeNet_airplane.yml",
    "../config/ShapeNet_piano.yml",
    "../config/ShapeNet_dresser.yml",
    "../config/ShapeNet_bookshelf.yml"
};

void ConfigManager::LoadConfig(std::string config_path) {
  LoadScaleTableParams(config_path, scale_params);
  ...
  data_path     = (std::string)fs["dataset_path"];
  // std::cout<<config_path<<" "<<data_path<<"---"<<std::endl;
