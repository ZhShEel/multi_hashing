...
void SavePly(CompactMesh& compact_mesh, std::string path) {
  ...
  checkCudaErrors(cudaMemcpy(triangles, compact_mesh.triangles(),
                             sizeof(int3) * compact_triangle_count,
                             cudaMemcpyDeviceToHost));
  checkCudaErrors(cudaMemcpy(triangle_scale, compact_mesh.scales(),
  ...
  for (uint i = 0; i < compact_vertex_count; ++i) {
    ...
    if(show_scale == 1){
      colorx = 1;  colory = 0;  colorz = 0;
    }
    else if(show_scale == 2){
      colorx = 1;  colory = 1;  colorz = 0;
    }
    else if(show_scale == 3){
      colorx = 0;  colory = 1;  colorz = 0;
    }
    else{
      colorx = 0;  colory = 0;  colorz = 1;
    }
