...
void Sensor::init(SensorParams &sensor_params) {
...
int Sensor::Process(cv::Mat &depth, cv::Mat &color, bool depth_no_change) {
  ...
  if (! depth_no_change) {
    ...
    ValidateDepth(depth, data_.depth_data, params_);
  }
...
// int Sensor::Process(pcl::PointCloud<pcl::PointXYZRGB> pc){
//   // if (! depth_no_change) {
//   //   ConvertDepthFormat(depth, data_.depth_buffer, data_.depth_data, params_);
//   // } else {
//   //   ValidateDepth(depth, data_.depth_data, params_);
