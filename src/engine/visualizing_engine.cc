...
VisualizingEngine::VisualizingEngine(std::string window_name,
                                     int width, int height, SensorParams &sensor_params) {
  Init(window_name, width, height, sensor_params);
}
...
void VisualizingEngine::Init(std::string window_name, int width, int height,
  ...
  camera_.set_projection_from_hartley(sensor_params.fx, sensor_params.cx, width,
                                      sensor_params.fy, sensor_params.cy, height);
...
void VisualizingEngine::set_light(gl::Lighting &light) {
  lighting_ = light;
}
...
void VisualizingEngine::BindMainProgram(
  ...
  if (!enable_color) {
    main_program_.Load(kShaderPath + "/directional_light_shading_vert.glsl",
                       gl::kVertexShader);
    main_program_.ReplaceMacro("LIGHT_COUNT", lighting_.light_count_str,
                               gl::kVertexShader);
    main_program_.Load(kShaderPath + "/directional_light_shading_frag.glsl",
                       gl::kFragmentShader);
    main_program_.ReplaceMacro("LIGHT_COUNT", lighting_.light_count_str,
                               gl::kFragmentShader);
    main_program_.Build();
    ...
    main_uniforms_.GetLocation(main_program_.id(), "light_dir", gl::kVector3f);
...
void VisualizingEngine::BindMainUniforms() {
  ...
  if (!enable_color_) {
    ...
    main_uniforms_.Bind("light_dir", lighting_.light_directions.data(),
                        lighting_.light_count);
    main_uniforms_.Bind("light_power", &lighting_.light_power, 1);
    main_uniforms_.Bind("ambient_coeff", &lighting_.material_ambient_coeff, 1);
    main_uniforms_.Bind("specular_coeff", &lighting_.material_specular_coeff,
                        1);
    main_uniforms_.Bind("diffuse_color", &lighting_.material_diffuse_color, 1);
    main_uniforms_.Bind("light_color", &lighting_.light_color, 1);
  }
}
...
void VisualizingEngine::BindMainData() {
  ...
  main_args_.BindBuffer(2, {GL_ELEMENT_ARRAY_BUFFER, sizeof(int), 3,
                            GL_UNSIGNED_INT},
...
void VisualizingEngine::RenderRayCaster(
  ...
  ray_caster_.Cast(hash_table,
                   ...
                   geometry_helper,
                   view);
  //printf("surface_image size: %d", ray_caster_.surface_image().rows);
  //cv::waitKey(-1);
  // cv::imshow("RayCasting", ray_caster_.surface_image());
  //    cv::imshow("RayCasting:Color",ray_caster_.color_image());
//  cv::imshow("RayCasting:Depth",ray_caster_.depth_image());

  cv::waitKey(1);
}

// join process visualization

void JoinProcessVis::Resize(uint count){
  pos_toShow = new float3[count];
  length_toShow = new int[count];
  join_state_toShow = new int[count];
  entry_count = count;
}
