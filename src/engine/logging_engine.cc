...
void LoggingEngine::Init(std::string path) {
  ...
  if (!time_stamp_file_.is_open()) {
    LOG(ERROR) << "Can't open mapping stamp file";
    return;
  }
  ...
  raycasting_time_file_.open(base_path_ + "/time_raycasting.txt");
...
void LoggingEngine::ConfigVideoWriter(int width, int height) {
  enable_video_ = true;
  //video_writer_.open(base_path_ + "/video.avi", cv::VideoWriter::CV_FOURCC('X', 'V', 'I', 'D'), 30, cv::Size(width, height));
}
...
void LoggingEngine::WritePly_Temp(CompactMesh &mesh, int f){
    switch(f){
        case 2:
            SavePly(mesh,base_path_+"/mesh_f2.ply");
            break;
        case 3:
            SavePly(mesh,base_path_+"/mesh_f3.ply"); 
            break;
        case 4:
            SavePly(mesh,base_path_+"/mesh_f4.ply");
            break;
        case 5:
            SavePly(mesh,base_path_+"/mesh_f5.ply");
            break;
        case 6:
            SavePly(mesh,base_path_+"/mesh_f6.ply");
            break;
        case 7:
            SavePly(mesh,base_path_+"/mesh_f7.ply");
            break;
        case 8:
            SavePly(mesh,base_path_+"/mesh_f8.ply");
            break;
    }
}
...
void LoggingEngine::WriteMappingTimeStamp(double alloc_time,
                                          ...
                                          double update_time,
                                          int frame_idx) {
  std::ofstream time_stamp_file__("tum1-shading/time_mapping1.txt",std::ios::app);
  time_stamp_file__ << frame_idx << " "
                   //<< "alloc time : "
                   << alloc_time * 1000 << " "//<< "ms "
                   //<< "collect time : "
...
void LoggingEngine::WriteMeshingTimeStamp(float time, float recycle_time, int frame_idx) {
  std::ofstream meshing_time_file__("tum1-shading/time_meshing1.txt",std::ios::app);
  meshing_time_file__ << frame_idx << " " << time << " "<<recycle_time<< "\n";
}

void LoggingEngine::WriteRayCastingTimeStamp(float time, int frame_idx){
  raycasting_time_file_ << frame_idx << " " << time << "\n";
}

void LoggingEngine::WriteMeshStats(int vtx_count, int tri_count, int frame_idx) {
  mesh_stats_file_ << frame_idx << " " << vtx_count << " " << tri_count << "\n";
}
